...
import { currentFirst, monthDiff, parseDate, durationLabel, dateLabel } from '../src/dates.js';
...
test('name periods put current first and former names newest to oldest', () => {
  const periods = [
  ...
  assert.deepEqual(currentFirst(periods).map(period => period.id), ['current', 'newest-former', 'oldest']);
  assert.equal(periods[0].id, 'oldest', 'sorting must not mutate canonical data');
});
